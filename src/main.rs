...
use rpc_helper::{get_vout, send_funding_transaction};
...
fn main() {
    ...
    let cat_tr_spend_info = create_cat_address(vec![cat_tx_out.clone()]).unwrap();
    ...
    let cat_vout = get_vout(&rpc, &cat_funding_txid, CAT_SPEND_AMOUNT);

    let inputs = TxIn {
        previous_output: OutPoint {
            txid: cat_funding_txid,
            vout: cat_vout.unwrap(),
        },
    ...
    let parent_tx = spend_cat(unsigned_tx, cat_tr_spend_info, prev_output_txout);

    let parent_serialized_tx = serialize_hex(&parent_tx);
    info!("\nSpending cat transaction...");

    info!("Parent transaction: {}", parent_serialized_tx);

    let txid = rpc.send_raw_transaction(parent_serialized_tx).unwrap();
